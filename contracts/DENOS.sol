...
contract DENOS is ERC20, Ownable {
  ...
  constructor(uint256 _initialSupply) payable ERC20("DENOS", "DNO") {
    maxWallet = _initialSupply / 100;
   ...
   * If the user is selling, and the swap threshold is met, it executes the tax.
   */
  function handleTax(
    address from,
    address to,
    ...
    if (!isExcluded(from) && !isExcluded(to)) {
      ...
      if (from == address(uniswapV2Pair)) {
        ...
        if (ethValue >= swapThreshold) {
          ...
          uint256 devETH = (ethGained * ((devTokens * 10 ** 18) / taxSum)) /
            10 ** 18;
